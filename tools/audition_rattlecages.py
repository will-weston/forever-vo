"""Build Sarvis's recipe reference and Rattling the Rattlecages recordings."""
import csv
import itertools
import json
import random
...
from config import BETA_BUILD, DATA_DIR, TOOLS_DIR, VOICES_DIR
from textclean import clean, chunk


def main():
    out = TOOLS_DIR / 'samples' / 'sarvis-rattlecages'
    out.mkdir(parents=True, exist_ok=True)
    # Wowhead NPC 1569: greetings 6042, farewells 6043, angry 6044.
    # Editorial assessment of quest 3901: stern instruction, impatient reminder, approving completion.
    # Start with neutral material for stern delivery; no explicit angry threats.
    allowed_kits = {6042, 6043}
    candidates = []
    ...
    if not choices:
        raise RuntimeError('Insufficient longer clips from Sarvis\'s voice set')
    def score(group):
    ...
    reference = out / 'sarvis-reference.wav'
    ref_wave = np.concatenate(pieces)
    sf.write(reference, ref_wave, 24000)
    manifest = {
        'npc': 1569, 'quest': 3901,
        'source': 'https://www.wowhead.com/classic/npc=1569/shadow-priest-sarvis#sounds',
        'tone_assessment': 'Stern challenge in offer, impatient sarcasm in progress, approving resolve in completion; start without angry clips.',
    ...
    for event in ['accept', 'progress', 'complete']:
        text = clean(quests[f'3901-{event}']['text']).replace('--', ', ').replace(' - ', ', ')
        random.seed(42)
        np.random.seed(42)
        ...
        path = out / f'rattling-the-rattlecages-{event}.wav'
